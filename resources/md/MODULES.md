...
| Stdlib | `stdlib/clojure/*.clj`, `stdlib/jolt/*.clj` | Lazily-loaded portable namespaces (string/set/walk/edn/pprint/zip/test/data) and Jolt's own: `jolt.ffi`, `jolt.fs` (babashka.fs-shaped), `jolt.process`, `jolt.socket`, `jolt.image`, `jolt.time`, `jolt.parser`, `jolt.infix`. | no |
| Scheme adapter | `host/scheme-adapter/`, `host/chez/scheme-adapter-runtime.ss` | The portability seam (#446): `CONTRACT.txt` names every non-R6RS binding the host may use on any target, `THREADS.md` designs the no-threads degradation, `chez.ss` asserts the contract gate-time, and the runtime file routes forbidden Chez-isms (process/GC/clock/introspection) through `sa-*` capability entry points. Enforced by `make portcheck` / `make adaptercheck`. | no |
