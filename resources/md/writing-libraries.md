...
`{:git/url … :git/sha …}` with a **full** SHA (`git fetch` can't resolve a short one). An optional `:deps/root` selects a subdirectory of the repo. Transitive dependencies from each library's own `deps.edn` are resolved too. Resolution is breadth-first, so a top-level coordinate always wins over a transitive one for the same library.
...
If a library reaches for a Java class, you can often supply it from Clojure; see [Host Interop](/docs/host-interop.html) for registering your own host-class shims from a library, the same mechanism Jolt's own libraries use to run unmodified Clojure code. A library that ships such shims declares the classes under [`:jolt/provides`](/docs/tools-deps.html#host_classes_a_library_provides) in its `deps.edn`, so a consumer's first reference to one of them loads the library without a `require`.
